...
char		server::memberChannelNumbers(const std::string& name)
{
	...
	void	server::start_in_channel(const std::vector<std::string> &split_cmd, std::vector<std::string>channelSplited, char mode, size_t fd, size_t i)
	{
		int id;
		int _fd;
	...
	void	server::addClientToInviteOnlyChannel(const std::vector<std::string> &split_cmd, std::vector<std::string>channelSplited, char mode, int fd, size_t i)
	{
		int id;
		int _fd;
		id = idChannelfd(channelSplited[i], _fd);
			...
			while (i < channelSplited.size())
			{
				if (checkChannelName(channelSplited[i]))
				{
					if (availableChannel(channelSplited[i]))
					{
						...
						if (mode & (1 << LIMIT))
							start_in_channel(split_cmd, channelSplited, mode, fd, i); // 
						else
							addClientToInviteOnlyChannel(split_cmd, channelSplited, mode, fd, i); //
					}
					else
					{
						std::cout << "3" << std::endl;
